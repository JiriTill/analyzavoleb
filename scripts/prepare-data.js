...
// Robustní join: CSV T4/T4p může mít v "OKRSEK" lokální číslo (např. 8001) nebo globální ID (např. 29587).
// Výstup vždy ukládáme pod klíčem = LOKÁLNÍ číslo okrsku.
...
function guessProp(obj, candidates) {
  ...
  for (const k of keys) { const kl = k.toLowerCase(); if (candidates.some((c) => kl.includes(c.toLowerCase()))) return k; }
  return null;
}
const asStr = (x) => (x == null ? null : String(x).trim());
const asInt = (x) => { const n = Number(String(x).replace(",", ".").replace(/\s+/g,"")); return Number.isFinite(n)? n : 0; };
...
function readT4(csvPath) {
  ...
  const OKR  = guessProp(s, ["OKRSEK", "ID_OKRSEK", "IDOKRSEK", "OKRSEK_ID"]);
  ...
  return rows.map((r) => ({
    OBEC: asStr(r[OBEC]),
    OKRSEK: asStr(r[OKR]),
    registered: asInt(r[VOL]),
    envelopes: asInt(r[VYD]),
    valid: asInt(r[PL]),
  }));
}
...
function readT4p(csvPath) {
  ...
  const OBEC = guessProp(s, ["OBEC", "KOD_OBEC", "CIS_OBEC"]);
  const OKR  = guessProp(s, ["OKRSEK", "ID_OKRSEK", "IDOKRSEK", "OKRSEK_ID"]);
  const KSTR = guessProp(s, ["KSTRANA", "KOD_STRANY", "KOD_SUBJEKTU", "KODSTRANA", "KOD_STR"]);
  const PHL  = guessProp(s, ["POC_HLASU", "HLASY"]);
...
function readCNS(zipDir) {
  ...
  const ordered = files.sort((a, b) => {
    ...
    const wa = /stran|strany|subjekt|kandid/i.test(pa) ? 0 : 1;
    const wb = /stran|strany|subjekt|kandid/i.test(pb) ? 0 : 1;
    return wa - wb;
  });
  for (const f of ordered) {
    ...
    const s = rows[0];
    const KSTR = guessProp(s, ["KSTRANA", "KOD_STRANY", "KOD_SUBJEKTU", "KODSTRANA", "KOD_STR"]);
    const NAME = guessProp(s, ["NAZ_STRANA", "NAZEV_STRANA", "NAZEV_SUBJEKTU", "NAZEV", "STRANA"]);
    if (KSTR && NAME) {
      const map = {};
      for (const r of rows) {
...
// -------------------- GeoJSON práce --------------------
const LOCAL_KEYS  = ["CISLO_OKRSKU", "cislo_okrsku", "CIS_OKRSEK", "OKRSEK", "cislo", "okrsek", "okrsek_cislo", "cislo_okrsku_text"];
// POZOR: záměrně NEOBSAHUJE obecné "id/ID", aby nedošlo k záměně s technickým ID feature!
const GLOBAL_KEYS = ["ID_OKRSEK", "ID_OKRSKU", "OKRSEK_ID", "IDOKRSEK", "id_okrsku", "okrsek_id"];
...
function harvestLocalGlobal(geo) {
  ...
  for (const f of geo.features || []) {
    const p = f.properties || {};
    const local  = featureVal(p, LOCAL_KEYS);
    const global = featureVal(p, GLOBAL_KEYS);
...
function buildResultsDirectLocal(t4, t4p, cns, localSet) {
  ...
  for (const local of localSet) {
    ...
    const agg = rows.reduce((acc, r) => { const code = r.party_code; if (!code) return acc; acc[code] = (acc[code] || 0) + (r.votes || 0); return acc; }, {});
...
function buildResultsViaPairs(t4, t4p, cns, pairs) {
  const t4ByGlobal = new Map(); for (const r of t4) { t4ByGlobal.set(String(r.OKRSEK || ""), r); }
  const t4pByGlobal = new Map(); for (const r of t4p) { const k = String(r.OKRSEK || ""); (t4pByGlobal.get(k) || t4pByGlobal.set(k, []).get(k)).push(r); }
...
(async function main() {
  ...
  try { const kz = await resolveKZ2024(); if (!kz.dataZip || !kz.cnsZip) throw new Error("kz2024: chybí zipy dat/číselníků"); await processElection("kz2024", kz); }
  catch (e) { console.warn("[WARN] KZ2024 přeskočeno:", e.message || e); }

  try { const kv = await resolveKV2022(); if (!kv.dataZip || !kv.cnsZip) throw new Error("kv2022: chybí zipy dat/číselníků"); await processElection("kv2022", kv); }
